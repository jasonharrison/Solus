...
def handle_data(self, data):  # start parsing
    ...
    if data[:4] == 'PING':
        ...
        if "o" in modes:  # oper!
            self.uidstore[uid] = {'nick': nick, 'user': user, 'host': host, 'realhost': realhost, 'account': account,
                                  'oper': True, 'modes': modes, 'channels': [], 'gecos': gecos, 'ip': ip,
                                  'server': server, 'uid': uid}
        else:  # not an oper
            self.uidstore[uid] = {'nick': nick, 'user': user, 'host': host, 'realhost': realhost, 'account': account,
                                  'oper': False, 'modes': modes, 'channels': [], 'gecos': gecos, 'ip': ip,
                                  'server': server, 'uid': uid}
    ...
    #:02KAAACJL NICK jg :1302376060
        ...
        for uid in self.serverstore[sid]['users']:
            self.log("network",
                     "squit: removing client " + uid + " due to " + self.serverstore[sid]['name'] + " splitting.")
            self.serverstore[self.uidstore[uid]['server']]['users'].remove(uid)
            del self.uidstore[uid]
    ...
    # an ugly hack lies ahead.
        ...
        self.serverstore[self.uplinkSID] = {"servername": self.uplinkservername, "SID": self.uplinkSID,
                                            "serverdesc": self.uplinkserverdesc, "users": []}
    elif split[1] == "ENCAP":
        if split[3] == "SU":
            uid = split[4].replace(":", "")
            try:
    ...
    # {1302922063.38} Recv: :10A ENCAP * SU 05CAAAISU :bikcmp
        ...
        cuid = split[0].strip(":")
        uid = split[2]
        self.sendLine(":" + self.mysid + " 311 " + cuid + " " + self.uidstore[uid]['nick'] + " " + self.uidstore[uid][
            'user'] + " " + self.uidstore[uid]['host'] + " * :" + self.uidstore[uid]['gecos'])
        self.sendLine(
            ":" + self.mysid + " 312 " + cuid + " " + self.uidstore[uid]['nick'] + " " + self.servername.replace("(H) ",
                                                                                                                 "") + " :" + self.serverdesc)
...
def createClient(self, cnick, cuser, chost, cgecos):
    ...
    if str(self.mysid) + str(myuid) in self.myclients:
        ...
        self.myclients.append(cuid)
    modes = "+ioS"
    self.sendLine(':' + str(self.mysid) + ' EUID ' + cnick + ' 0 ' + str(
        time.time()) + ' ' + modes + ' ' + cuser + ' ' + chost + ' 0.0.0.0 ' + cuid + ' 0.0.0.0 0 :' + cgecos)
    self.uidstore[cuid] = {'nick': cnick, 'user': cuser, 'host': chost, 'realhost': chost, 'account': "", 'oper': True,
                           'modes': modes, 'channels': [], 'gecos': cgecos, 'ip': "", 'server': self.mysid, 'uid': cuid}
...
def kill_user(self, killer, killed, reason):
    ...
    self.sendLine(
        ":" + killer + " KILL " + killed + " :" + cserver + "!" + chost + "!" + cuser + "!" + cnick + " (" + reason + ")")
